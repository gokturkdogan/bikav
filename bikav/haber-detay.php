...
$sorgu=$vt->prepare('SELECT *FROM vakifhaberleri WHERE id=?');
$sorgu->execute(array($id));
$haberler=$sorgu-> fetch(PDO::FETCH_ASSOC); 

$sorgu1=$vt->prepare('SELECT *FROM vakifhaberleri ORDER BY id DESC LIMIT 5');
$sorgu1->execute();
					...
					<div class="sidebar-section">
						<div class="recent-posts">
							...
							<?php foreach($haberbenzer as $haber){?>
								<li><a href="haber-detay.php?id=<?= $haber->ID ?>"><?= $haber->baslik ?></a></li>
								<?php } ?>
							</ul>
						</div>
						<div class="tag-section">
							...
							<ul>
							<?php foreach($haberbenzer as $haber){?>
								<li><a href="vakifhaberleri.php"><?= $haber->anahtar ?></a></li>
								<?php } ?>
							</ul>
						</div>
					</div>
